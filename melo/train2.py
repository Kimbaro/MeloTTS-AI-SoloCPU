...
def run(config: str, model: str, pretrain_G: str, pretrain_D: str, pretrain_dur: str):
    ...
    print(f"경로체크! -> {hps.model_dir}")
    device = "cuda" if torch.cuda.is_available() else "cpu"
    # CPU 환경이면 분산 학습을 사용하지 않음
    if not torch.cuda.is_available():
        print("CPU 환경: 단일 프로세스로 실행")
        rank = 0
        world_size = 1
        n_gpus = world_size
    else:
        ...
        # 환경 변수 기본값 설정
        os.environ.setdefault("WORLD_SIZE", "1")  # 총 프로세스 수
        os.environ.setdefault("RANK", "0")  # 현재 프로세스의 rank
        os.environ.setdefault("LOCAL_RANK", "0")  # 단일 프로세스이므로 LOCAL_RANK는 0
        os.environ.setdefault("MASTER_ADDR", "127.0.0.1")  # 마스터 노드 주소
        os.environ.setdefault("MASTER_PORT", "29500")  # 사용할 포트

        local_rank = 0  # 단일 프로세스 실행
        dist.init_process_group(
            backend="gloo",
            init_method="env://",  # Due to some training problem, we proposed to use gloo instead of nccl.
            rank=local_rank,
            world_size=1,  # CPU는 단일 프로세스
        )
    ...
    print(f"check!!! -> {hps.pretrain_G}")
    print(f"check!!! -> {hps.pretrain_D}")
    print(f"check!!! -> {hps.pretrain_dur}")
